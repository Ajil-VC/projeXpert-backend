...
import { IGetRoleWithIdUsecase, ICreateRoleUsecase, IDeleteRoleUsecase, IGetRolesUsecase, IUpdateProfileUsecase, IUpdateRoleUsecase } from "../../config/Dependency/user/user.di";
...
export class userController implements IUserController {
    ...
    constructor(
        private _updateProfilePic: IUpdateProfileUsecase,
        private _createRoleUse: ICreateRoleUsecase,
        private _getRolesUse: IGetRolesUsecase,
        private _getRoleWithId: IGetRoleWithIdUsecase,
        private _deleteRoleUse: IDeleteRoleUsecase,
        private _updateRole: IUpdateRoleUsecase
    ) { }
    ...
    updateRoleData = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            const role = await this._getRoleWithId.execute(roleId);
            if (!role.canMutate) {
                res.status(HttpStatusCode.CONFLICT).json({ status: false, message: "This role cannot be edited." });
                return;
            }

            const result = await this._updateRole.execute(roleName, permissions, description, roleId);
            if (!result) {
    ...
    deleteRole = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            const result = await this._deleteRoleUse.execute(roleId);
            if (!result) {
    ...
    getRoles = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            const roles = await this._getRolesUse.execute(req.user.companyId);
    ...
    createRole = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            const result = await this._createRoleUse.execute(roleName, permissions, description, req.user.companyId);
    ...
    updateProfile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            const file = files[0];
            const result = await this._updateProfilePic.execute(file, req.user.id, name);
